...
void sin_cos(audio::s16PCMFrame *frame)
{
    ...
    for (int i = 0; i < 1024; ++i)
    {
        ...
        frame->setLeftSoundTrackAt(i, static_cast<int16_t>(std::sin(static_cast<double>(loop) / 256) * sum / 2));
        frame->setRightSoundTrackAt(i, static_cast<int16_t>(std::cos(static_cast<double>(loop) / 256) * sum / 2));
    }
    ++loop;
};


void dumb(audio::s16PCMFrame *frame)
{
    static int loop = 0;
    for (int i = 0; i < 1024; ++i)
    {
...
void sound_mix(audio::s16PCMFrame *frame ,audio::s16PCMFrame *another_frame)
{
    ...
    {
        int32_t adjust;
        adjust = static_cast<int32_t>( (frame->getLeftSoundTrackAt(i) + another_frame->getLeftSoundTrackAt(i)) * factor);
        if(adjust > INT16_MAX)
        {
            factor = INT16_MAX / static_cast<double>(adjust);
            adjust = INT16_MAX;
...
double average(audio::s16PCMFrame *frame)
{
    double aver = 0;
    for( int i = 0; i < 1024; i++)
    {
...
void roate(audio::s16PCMFrame *frame)
{
    for(int i = 0;i < 1024;i++)
    {
